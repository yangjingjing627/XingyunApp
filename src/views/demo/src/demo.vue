...
<bq-page>
  ...
  <bq-content ref="content" @scroll="onScroll" class="bg">
    <div style="margin: 0 38px;">
      <bq-wx-upload @on-success="onSuccess" :single="true"></bq-wx-upload>
      <bq-wx-upload @on-success="onSuccess" v-model="img2" :count="5" ></bq-wx-upload>
    </div>
    <br>
    {{ uploadValues }} {{ img2 }}
    ...
    <bq-button type="primary" size="small" @click="onParabola" ref="btn">抛物线</bq-button>
    <br>
...
export default {
  ...
  async created() {
    ...
    let cate = await this.TestService.getCate()
    console.log('CATE', cate)
    let res = await this.TestService.getByTopicId({topicId: 15})
